...
class Simple_Pipeline_Window:
    ...
    def create_widgets(self):
        ...
        # Botón para verificar bóveda
        self.verify_vault_button = tk.Button(self.controls_frame, text="Verificar Bóveda", command=self.verify_vault_simple)
        self.verify_vault_button.pack(side=tk.LEFT, padx=5)
    ...
    def load_sign_and_verify_file(self):
        ...
        try:
            ...
            # Obtener la clave de la bóveda
            private_key = processor.vault.keys[processor.key_index]
    ...
    def verify_vault_simple(self):
        """
        Verificar firma de bóveda de forma simple
        """
        # Seleccionar archivo firmado
        archivo_firmado = filedialog.askopenfilename(
            title="Seleccionar archivo firmado (.bin)",
            filetypes=[("Archivos firmados", "*.bin"), ("Todos", "*.*")]
        )
        ...
        try:
            ...
            # Verificar y mostrar resultado simple
            es_valida = verificador.verificar_firma(archivo_firmado)
            
        except Exception as e:
            messagebox.showerror("Error", f"Error verificando archivo: {str(e)}")
